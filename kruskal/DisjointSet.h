...
inline DisjointSet::Node::Node() {
    _parent = this;
    _depth = 0;
}

// getter- and setter-methods
inline void DisjointSet::Node::set_parent(Node *parent) {
    _parent = parent;
}

inline void DisjointSet::Node::increment_depth() {
    _depth++;
}

inline DisjointSet::Node *DisjointSet::Node::get_parent() const {
    return _parent;
}

inline int DisjointSet::Node::get_depth() const {
    return _depth;
}
...
inline void DisjointSet::make_sets(int num_new_sets) {
    _nodes.resize(_nodes.size() + num_new_sets);
}

// the -find- method from the lecture
inline DisjointSet::Node *DisjointSet::find(Node *node) {
...
inline void DisjointSet::unite(int a, int b) {
    Node *a_ptr = &_nodes[a];
    Node *b_ptr = &_nodes[b];
...
inline bool DisjointSet::set_equals(int a, int b) {
    if (find(&_nodes[a]) == find(&_nodes[b]))
        return true;
    return false;
}

inline int DisjointSet::get_num_nodes() const {
